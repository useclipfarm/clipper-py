...
class FTCEventsClient:
    ...
    def fetch(self, path, **params):
        ...
        if self.verbose:
            print("fetch", url)
        r = self.session.get(
            url, headers={"Authorization": "Basic " + self._b64}, params=params
        )
        r.raise_for_status()
        return r.json()
    ...
    def get_season():
        ...
        kickoff = [
            d
            for d in [
                datetime.datetime(year=year, month=9, day=i) for i in range(8, 15)
            ]
            if d.weekday() == 5
        ][0]
        if kickoff > today:
    ...
    def get_schedule(self, eventcode):
        ...
        for m in qual:
            ...
            blue = []
            for team in sorted(
                m["teams"], key=lambda x: x["station"]
            ...
            m["matchSummary"] = (
                f"{name} --- {', '.join(red)} vs. {', '.join(blue)} -- {m['scoreRedFinal']}-{m['scoreBlueFinal']} {wincode}"
            )
            match_map[name] = m
