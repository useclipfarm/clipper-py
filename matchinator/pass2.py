...
from . import consts, match_result, matchers, pass1, util
...
def create_sample(group: List[pass1.Pass1EventMatch]) -> SampleInfo:
    ...
    return SampleInfo(
        group=sample, avg_match_start=np.mean(starts), score=np.var(starts)
    )


def combine_matches(edata: pass1.Pass1EventData, seed=0) -> List[Pass2EventMatch]:
    random.seed(seed)
    groups: List[List[pass1.Pass1EventMatch]] = coalese_groups(
        filter_groups(coalese_groups(edata.matches))
    )
    all_matches = []
    ...
    for match_group in groups:
        ...
        soln = min(
            [create_sample(match_group) for i in range(100)], key=lambda x: x.score
        )
...
def clip_match(
    ...
    fname_template: str,
    offset=0,
    pass1_data: pass1.Pass1EventData = None,
):
    match_name = p.name
    match_numer = 0
    tiebreaker_num = 1
    ...
    if pass1_data is not None:
        for k, v in pass1_data.match_result_map.items():
            if not k.startswith("https://ftc.events"):
                continue
            if match_name.startswith("da Vinci") and k.endswith(
                f"FTCCMP1/playoff/{match_numer}/{tiebreaker_num}"
            ):
                results_screen = v
                break
            if match_name.startswith("Qualification") and k.endswith(
                f"qualifications/{match_numer}"
            ):
                results_screen = v
                break
            if match_name.startswith("Match") and k.endswith(
                f"playoff/{match_numer}/{tiebreaker_num}"
            ):
                results_screen = v
                break
    ...
    if results_screen is None:
        ...
        results_start = (
            results_screen.start_ts
            - consts.PRE_RESULT_FINE
            - consts.PRE_RESULT_COARSE
            - offset
        )
        results_end = (
            min(
                results_screen.start_ts + consts.POST_SCORE_DETECT,
                results_screen.end_ts - 2,
            )
            - offset
        )
        ...
        subprocess.check_call(
            [
                ...
                src,
                "-ss",
                f"{consts.PRE_RESULT_COARSE:.03f}",
                "-c:v",
                "libx264",
                "-crf",
                "17",
