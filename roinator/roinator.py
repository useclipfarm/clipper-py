...
class ROI:
    ...
    def redraw(self, window: sg.Window, line_color):
        ...
        if self.visible:
            self.tk_handle = graph.draw_rectangle(
                (self.x, self.y),
...
class Model:
    ...
    def from_file(self, fname, img_size):
        ...
        if (data["img_w"], data["img_h"]) != img_size:
            sg.popup_ok(
                f"Warning: image size {img_size} != {(data['img_w'], data['img_h'])}"
            )
    ...
    def reload_names(self):
        self.roilist.update(
            values=[roi.name for roi in self.roi],
            set_to_index=self.roilist.get_indexes(),
        )
...
def main(fname):
    ...
    mouse_tool = [
        [sg.T("ROI click mode", enable_events=True)],
        ...
        [sg.T("Load/save")],
        [
            sg.InputText(visible=False, enable_events=True, key="!SaveLayoutPath"),
            sg.FileSaveAs(
                "Save Layout", key="!SaveLayout", file_types=(("JSON file", ".json"),)
            ),
        ],
    ...
    rect_select = [
        [sg.T("Regions of interest:")],
        [
            sg.Listbox(
                [], key="!ROIList", size=(24, 15), expand_y=True, enable_events=True
            )
        ],
    ]
    ...
    gen_tool = [
        ...
        [
            sg.B("Flip horiz", key="!ROIXflip", enable_events=True),
            sg.B("Flip vert", key="!ROIYflip", enable_events=True),
        ],
    ]
    ...
    while True:
        ...
        if event == "!ROIGraph":
            ...
            if None not in (start_point, end_point):
                if values["!MoveROI"]:
                    if roi and (
                        (0 <= roi.x + delta_x)
                        and (0 <= roi.y + delta_y)
                        and (roi.x + roi.w + delta_x < img_size[0])
                        and (roi.y + roi.h + delta_y < img_size[1])
                    ):
                        # update boxes
                        roi.x += delta_x
                        roi.y += delta_y
                    ...
                    end_point = (
                        max(0, min(end_point[0], img_size[0] - 1)),
                        max(0, min(end_point[1], img_size[1] - 1)),
                    )
                    prior_rect = graph.draw_rectangle(
                        start_point,
                        end_point,
                        line_color=SELECT_LINE_COLOR,
                        line_width=3,
                    )
            ...
            if values["!DrawROI"] and start_point is not None:
                ...
                graph.delete_figure(prior_rect)
                prior_rect = graph.draw_rectangle(
                    start_point, end_point, line_color=SELECT_LINE_COLOR, line_width=3
                )
                graph.update()
            ...
            window["!ROIInfo"].update(
                value=f"grabbed rectangle from {start_point} to {end_point}"
            )
            ...
            new_val = roi.set_on_validate_int(
                "x", values["!ROIXcoord"], 0, img_size[0] - roi.w - 1
            )
            ...
            new_val = roi.set_on_validate_int(
                "y", values["!ROIYcoord"], 0, img_size[1] - roi.h - 1
            )
            if new_val is not None:
            ...
            new_val = roi.set_on_validate_int(
                "w", values["!ROIWidth"], 0, img_size[0] - roi.x - 1
            )
            if new_val is not None:
            ...
            new_val = roi.set_on_validate_int(
                "h", values["!ROIHeight"], 0, img_size[1] - roi.y - 1
            )
            if new_val is not None:
            ...
            layout_fname = sg.popup_get_file(
                "Open layout file", file_types=(("JSON layout files", ".json"),)
            )
